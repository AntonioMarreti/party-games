...
function getGameLibrary() {
    return [
        ...
        'erudition' => ['edible_inedible', 'alchemy']      
    ];
}
...
function handleGameAction($pdo, $room, $user, $postData) {
    ...
    if ($type === 'submit_result') {
        ...
        if (isset($state['round_results'][$userId])) return ['status' => 'ok'];
        ...
        if (!isset($state['scores'][$userId])) $state['scores'][$userId] = 0;
...
function generateTaskData($type) {
    ...
    if ($type === 'math_blitz') {
        ...
        if ($op === '*') $opts = [$ans, $ans + $a, $ans - $b, $ans + rand(2, 10)];
    ...
    if ($type === 'greater_less') {
        ...
        $item1 = $genExpr();
        $item2 = $genExpr();
        while($item1['val'] == $item2['val']) $item2 = $genExpr();
    ...
    if ($type === 'color_chaos') {
        $c = [
            ...
            'orange'=>'Оранжевый', 'purple'=>'Фиолетовый', 'pink'=>'Розовый', 'black'=>'Черный'
        ];
        $k = array_keys($c); $tk = $k[array_rand($k)]; $ck = $k[array_rand($k)];
    ...
    if ($type === 'odd_one_out') {
        // Pairs of [Majority, Minority] using Bootstrap Icons
         ...
         if (rand(0,1)) { $maj = $p[0]; $min = $p[1]; } 
         else { $maj = $p[1]; $min = $p[0]; }

         // Увеличиваем до 16 элементов (сетка 4x4)
         $opts = array_fill(0, 15, $maj); $opts[] = $min; shuffle($opts);
    ...
    if ($type === 'dice_sum') {
        ...
        $sum = 0; $icons = [];
        for ($i=0; $i<$count; $i++) {
            $v = rand(1, 6); $sum += $v; $icons[] = $dice_map[$v];
        }
    ...
    if ($type === 'alchemy') {
        ...
        shuffle($wrong);
        $opts = [$r['res'], $wrong[0], $wrong[1], $wrong[2]]; shuffle($opts);
    ...
    if ($type === 'edible_inedible') {
        $items = [
            ['name' => 'Кирпич', 'type' => 'no'], ['name' => 'Яблоко', 'type' => 'yes'],
            ['name' => 'Гвоздь', 'type' => 'no'], ['name' => 'Бургер', 'type' => 'yes'],
            ['name' => 'Мыло', 'type' => 'no'],   ['name' => 'Суп', 'type' => 'yes'],
            ['name' => 'Кактус', 'type' => 'no'], ['name' => 'Пицца', 'type' => 'yes'],
            ['name' => 'Камень', 'type' => 'no'], ['name' => 'Банан', 'type' => 'yes'],
            ['name' => 'Носок', 'type' => 'no'],  ['name' => 'Стейк', 'type' => 'yes'],
            ['name' => 'Стекло', 'type' => 'no'], ['name' => 'Морковь', 'type' => 'yes'],
            ['name' => 'Батарейка', 'type' => 'no'], ['name' => 'Клубника', 'type' => 'yes'],
            ['name' => 'Шина', 'type' => 'no'],   ['name' => 'Сыр', 'type' => 'yes'],
            ['name' => 'Монета', 'type' => 'no'], ['name' => 'Пончик', 'type' => 'yes'],
            ['name' => 'Лампочка', 'type' => 'no'], ['name' => 'Арбуз', 'type' => 'yes'],
            ['name' => 'Песок', 'type' => 'no'],  ['name' => 'Брокколи', 'type' => 'yes'],
            ['name' => 'Бумага', 'type' => 'no'], ['name' => 'Курица', 'type' => 'yes'],
            ['name' => 'Краб', 'type' => 'yes'],  ['name' => 'Мел', 'type' => 'no'],
            ['name' => 'Шоколад', 'type' => 'yes'], ['name' => 'Ключ', 'type' => 'no'],
            ['name' => 'Пульт', 'type' => 'no'],  ['name' => 'Малина', 'type' => 'yes'],
            ['name' => 'Флэшка', 'type' => 'no'], ['name' => 'Авокадо', 'type' => 'yes'],
            ['name' => 'Кроссовки', 'type' => 'no'], ['name' => 'Манго', 'type' => 'yes'],
            ['name' => 'Ложка', 'type' => 'no'],  ['name' => 'Огурец', 'type' => 'yes'],
            ['name' => 'Вилка', 'type' => 'no'],  ['name' => 'Помидор', 'type' => 'yes'],
            ['name' => 'Шкаф', 'type' => 'no'],   ['name' => 'Черника', 'type' => 'yes'],
            ['name' => 'Подушка', 'type' => 'no'], ['name' => 'Пельмени', 'type' => 'yes'],
            ['name' => 'Очки', 'type' => 'no'],   ['name' => 'Шаурма', 'type' => 'yes'],
            ['name' => 'Чайник', 'type' => 'no'], ['name' => 'Блины', 'type' => 'yes'],
            ['name' => 'Зонт', 'type' => 'no'],   ['name' => 'Мед', 'type' => 'yes'],
            ['name' => 'Утюг', 'type' => 'no'],   ['name' => 'Творог', 'type' => 'yes'],
            ['name' => 'Книга', 'type' => 'no'],  ['name' => 'Редис', 'type' => 'yes'],
            ['name' => 'Часы', 'type' => 'no'],   ['name' => 'Зефир', 'type' => 'yes'],
            ['name' => 'Ножницы', 'type' => 'no'], ['name' => 'Абрикос', 'type' => 'yes'],
            ['name' => 'Пуговица', 'type' => 'no'], ['name' => 'Грибы', 'type' => 'yes'],
            ['name' => 'Карандаш', 'type' => 'no'], ['name' => 'Ананас', 'type' => 'yes'],
            ['name' => 'Ластик', 'type' => 'no'],  ['name' => 'Сырники', 'type' => 'yes'],
            ['name' => 'Телефон', 'type' => 'no'], ['name' => 'Котлета', 'type' => 'yes'],
            ['name' => 'Кольцо', 'type' => 'no'],  ['name' => 'Груша', 'type' => 'yes'],
            ['name' => 'Кофемолка', 'type' => 'no'], ['name' => 'Баклажан', 'type' => 'yes'],
            ['name' => 'Ботинок', 'type' => 'no'], ['name' => 'Облепиха', 'type' => 'yes'],
            ['name' => 'Скрепка', 'type' => 'no'], ['name' => 'Креветка', 'type' => 'yes'],
            ['name' => 'Кисточка', 'type' => 'no'], ['name' => 'Апельсин', 'type' => 'yes'],
            ['name' => 'Линейка', 'type' => 'no'], ['name' => 'Макароны', 'type' => 'yes'],
            ['name' => 'Молоток', 'type' => 'no'], ['name' => 'Йогурт', 'type' => 'yes'],
            ['name' => 'Краска', 'type' => 'no'],  ['name' => 'Киви', 'type' => 'yes']
        ];
    ...
    if ($type === 'count_objects') {
        ...
        shuffle($others);
        for($i=0; $i<5; $i++) $grid[] = $others[0];
        for($i=0; $i<4; $i++) $grid[] = $others[1];
        shuffle($grid);
